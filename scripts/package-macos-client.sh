...
(
  cd "$repo_dir"
  # `portal-dist` is generated by src-tauri/build.rs and is not committed, but
  # `cargo tauri build` validates frontendDist before cargo runs any build
  # script. Materialize it first so a clean checkout can build at all.
  cargo check --locked -p portal
  if [[ "$signing_mode" == adhoc ]]; then
    # `-` is the ad-hoc pseudo-identity, not an Apple credential, so it does not
    # violate the guard above. Tauri must apply it *during* bundling: the DMG is
    # built from the .app, so signing afterwards would publish a DMG containing
    # an unsigned bundle that arm64 macOS refuses to execute.
