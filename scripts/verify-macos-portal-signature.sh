...
usage() {
  printf 'Usage: %s --dmg /absolute/path/Portal-VERSION-arm64.dmg --signing adhoc|developer-id [--expected-team-id TEAMID] [--expected-version VERSION]\n' "$0"
}
...
while (($#)); do
  case "$1" in
    ...
    --signing) [[ $# -ge 2 ]] || die "$1 requires a value"; signing_mode="$2"; shift 2 ;;
    --help|-h) usage; exit 0 ;;
...
case "$signing_mode" in
  developer-id)
    [[ "$expected_team_id" =~ ^[A-Z0-9]{10}$ ]] \
      || die 'expected Apple TeamIdentifier is required for developer-id mode'
    ;;
  adhoc)
    [[ -z "$expected_team_id" ]] \
      || die '--expected-team-id is meaningless for an ad-hoc release'
    ;;
...
if [[ "$signing_mode" == developer-id ]]; then
  ...
  # The ad-hoc signature is what lets arm64 macOS run the binary at all. It
  # asserts nothing about origin, so also prove no Apple identity crept in.
